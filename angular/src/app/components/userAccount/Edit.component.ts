...
export class UserAccountEdit {
  ...
  edit() {
    ...
    data.RoleId = Array.from(document.querySelectorAll('[name="RoleId"]:checked')).map((e: any) => e.value)
    data = this.util.getFormData(data)
    this.UserAccountService.edit(this.route.snapshot.params['id'], data).subscribe(() => {
