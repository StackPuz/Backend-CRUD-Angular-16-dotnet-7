          ...
          <form ngNativeValidate method="post" (submit)="create()" enctype="multipart/form-data">
            <div class="row">
              ...
              <div class="form-group col-md-6 col-lg-4">
                ...
                <select id="product_brand_id" name="BrandId" class="form-control form-control-sm" [(ngModel)]="product.BrandId" required>
                  <option *ngFor="let brand of brands" value="{{brand.Id}}" [selected]="product != null && product.BrandId == brand.Id">{{brand.Name}}</option>
                </select>
